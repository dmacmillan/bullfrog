        ...
        for mrna_name in to_analyze:
            ...
            if compare_blocks:
                if ((gmap_num_blocks == maker_num_blocks) and (gmap_blocksizes == maker_blocksizes)):
                    # logging.info('Gmap is correct: {}'.format(mrna_name))
                    # Use Gmap information
                    gmap_tstarts = [x[0] for x in kwargs['gffs']['gmap'][mrna_name]['exon']]
                    pass
                elif ((blat_num_blocks == maker_num_blocks) and (blat_blocksizes == maker_blocksizes)):
                    # logging.info('Blat is correct: {}'.format(mrna_name))
                    # Use Blat information
                    pass
                else:
...
# _file is output from getProteinFromGff.py
def parse_proteins(_file):
    result = {}
...
if __name__ == '__main__':
    ...
    # Load proteins
    proteins = {
        'maker': parse_proteins(args.maker_proteins),
        'gmap': parse_proteins(args.gmap_proteins),
        'blat': parse_proteins(args.blat_proteins)
    }
